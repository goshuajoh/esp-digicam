...
#define WIFI_PASS "Espressif!1234"
...
#define EXAMPLE_ESP_WIFI_SSID      "ESP_1"
#define EXAMPLE_ESP_WIFI_PASS      "Espressif!123"
...
void wifi_provisioning_init()
{
    ESP_LOGI(TAG, "Starting BLE Wi-Fi provisioning");
...
void wifi_reset_and_provision()
{
    ESP_LOGI(TAG, "Resetting Wi-Fi and starting BLE provisioning");

    // Stop Wi-Fi
    esp_wifi_stop();

    // Reset Wi-Fi configurations
    esp_wifi_restore();
...
void stream_to_lcd_task(void *pvParameters)
{
    while (true)
    {
        if (!freeze_frame)
        {
            ...
            if (fb)
            {
                ...
                int start_x = (vga_width - lcd_width) / 2;
                int start_y = (vga_height - lcd_height) / 2;

                for (int y = 0; y < lcd_height; y++)
                {
                    esp_lcd_panel_draw_bitmap(panel_handle, 0, y, lcd_width, y + 1,
                                              &((uint16_t *)fb->buf)[(start_y + y) * vga_width + start_x]);
                }
        ...
        {
            vTaskDelay(100 / portTICK_PERIOD_MS); // Delay while frame is frozen
        }
    }
}
...
void capture_and_send_image_task(void *pvParameters)
{
    ...
    // for (int y = 0; y < lcd_height; y++)
    // {
    //     // Directly draw the required section from the framebuffer
    //     esp_lcd_panel_draw_bitmap(panel_handle, 0, y, lcd_width, y + 1,
    //                               &((uint16_t *)fb->buf)[(start_y + y) * vga_width + start_x]);
    // }
    ...
    while (true)
    {
        if (gpio_get_level(BOOT_BUTTON_GPIO) == 0)
        {
            ...
            if (gpio_get_level(BOOT_BUTTON_GPIO) == 0)
            {
                ...
                xTaskCreate(&capture_and_send_image_task, "capture_and_send_image_task", 8192, NULL, 5, NULL);
...
void app_main()
{
    ...
    // Configure the camera according to your setup
    // Example:
    ...
    // Create the task to capture and send image
    xTaskCreate(button_task, "button_task", 8 * 1024, NULL, 5, NULL);
    // xTaskCreate(&capture_and_send_image_task, "capture_and_send_image_task", 8192, NULL, 5, NULL);
    xTaskCreate(stream_to_lcd_task, "stream_to_lcd_task", 8 * 1024, NULL, 5, NULL);
}
